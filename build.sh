...
function download_package_max_upstream_no_deps() {
	...
	while IFS=';' read -r name version file depends; do
		...
		# Some PDM companion packages, such as UI/docs, are versioned separately
		# from the main source package. Use the newest upstream version that is
		# not newer than the requested PDM source version.
		upstream=${version%%-*}
...
PROXMOX_DM_GIT=$(resolve_commit_for_package_version "${PROXMOX_DM_VER}" proxmox-datacenter-manager proxmox-datacenter-manager) || true
